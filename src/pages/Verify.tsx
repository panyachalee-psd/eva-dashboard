...
export default function VerifyToken() {
  ...
  useEffect(() => {
    ...
    if (!token) {
      // ❌ no token → mark invalid
      setIsValid(false);
      setLoading(false);
      return;
    }

    // ✅ token exists → store it and go to dashboard
    localStorage.setItem("auth_token", token);

    setIsValid(true);
    setLoading(false);

    // navigate after short delay to show spinner smoothly
    setTimeout(() => {
...
//         }
//       } catch {
//         // Axios interceptor already shows popup
...
//   // ❌ Error State → show custom 404
//   if (error) {
//     return <NotFound />;
//   }

//   // Should not render anything, user gets redirected
//   return null;
// }
